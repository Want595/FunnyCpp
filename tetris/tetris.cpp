...
#define Game_X 29	//游戏区行数
#define Game_Y 20	//游戏区列数
#define WALL  2
#define KONG 0
...
struct Face
{
	int data[Game_X][Game_Y + 10];	//结构体定义时数组均已被置零
	int color[Game_X][Game_Y + 10];
}face;
struct Blocks
{
	int space[4][4];
...
int main()
{
	system("Tetris");    //设置cmd窗口标题
	system("mode con lines=29 cols=60");    //设置cmd窗口高度、宽度
	HWND hwndConsole = GetConsoleWindow();    // 获取控制台窗口句柄
    RECT rectConsole;
    GetWindowRect(hwndConsole, &rectConsole);    // 获取控制台窗口的位置和尺寸

    int screenW = GetSystemMetrics(SM_CXSCREEN);    // 获取屏幕的宽度
    int screenH = GetSystemMetrics(SM_CYSCREEN);    // 获取屏幕的高度

    int consoleW = rectConsole.right - rectConsole.left;    // 计算控制台窗口的宽度
    int consoleH = rectConsole.bottom - rectConsole.top;    // 计算控制台窗口的高度

    int consoleX = (screenW - consoleW) / 2;    // 计算控制台窗口的左上角x坐标
    int consoleY = (screenH - consoleH) / 2;    // 计算控制台窗口的左上角y坐标

    SetWindowPos(hwndConsole, NULL, consoleX, consoleY, 0, 0, SWP_NOSIZE | SWP_NOZORDER);    // 设置控制台窗口位置
	color(7);    //改变输出字符的颜色
	Hide_Cursor();    //隐藏光标
	Read_File();    //读取最高得分
	Inter_Face();    //初始化界面
...
#pragma warning(disable:4996)    //对 Visual Studio 2019 进行的警告消除，IDE不是Visual Studio可以删去
	FILE *fp;
	fp = fopen("最高分.txt", "r+");	//r+ 以可读写方式打开文件，该文件必须存在
	if (fp == NULL)
	{
		fp = fopen("最高分.txt", "w+");
...
void Inter_Face()    //初始化界面
{
	...
	for (i = 0; i < Game_X; i++)
	{
		for (j = 0; j < Game_Y + 10; j++)
		{
			if (j == 0 || j == Game_Y - 1 || j == Game_Y + 9)
			{
				face.data[i][j] = WALL;
				gotoxy(2 * j, i);
				printf("■");
	...
	gotoxy(2 * Game_Y + 4, Game_X - 18);
	printf("左移：←");
 
	gotoxy(2 * Game_Y + 4, Game_X - 16);
	printf("右移：→");
 
	gotoxy(2 * Game_Y + 4, Game_X - 14);
	printf("旋转：space");
 
	gotoxy(2 * Game_Y + 4, Game_X - 12);
	printf("暂停: S");
 
	gotoxy(2 * Game_Y + 4, Game_X - 10);
	printf("退出: ESC");
 
	gotoxy(2 * Game_Y + 4, Game_X - 8);
	printf("重新开始:R");
 
	gotoxy(2 * Game_Y + 4, Game_X - 6);
	printf("最高纪录:%d", max);
 
	gotoxy(2 * Game_Y + 4, Game_X - 4);
	printf("分数：%d", grade);
}
...
{
	int space_z = 0, n, x = Game_Y / 2 - 2, y = 0, t = 0, i, j, ch;	//x,y初始值即屏幕顶端掉落blocks处
	Draw_Kong(nn, space_z, Game_Y + 3, 4);
	n = nn;	//因为右上角要显示下一块blocks，因此这里先记录下当前blocks的base
	nn = rand() % 7;	//随机生成下一块blocks的base
	color(nn);
	Draw_Blocks(nn, space_z, Game_Y + 3, 4);
	while (1)
	{
		...
		else
		{
			...
			case ESC:
				system("cls");
				gotoxy(Game_Y, Game_X / 2);
				printf("   ---游戏结束---");
				gotoxy(Game_Y, Game_X / 2 + 1);
				printf("---请按任意键退出---");
				system("pause>nul");
				exit(0);
			case 'S':
...
#pragma warning(disable:4996)    //对 Visual Studio 2019 进行的警告消除，IDE不是Visual Studio可以删去
	int i, j, sum, m, n;
	for (i = Game_X - 2; i > 4; i--)
	{
		sum = 0;
		for (j = 1; j < Game_Y - 1; j++)
		{
			sum += face.data[i][j];
		}
		if (sum == 0)
